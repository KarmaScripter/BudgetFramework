...
namespace BudgetFramework
{
    ...
    public abstract class ExcelSettings
    {
        ...
        public virtual void SetExcelForm( ExcelPackage spreadSheet, DataTable dataTable )
        {
            if( spreadSheet != null
               && dataTable?.Rows?.Count > 0 )
            {
                try
                {
                    //spreadSheet.Workbook.ListObjects.Clear( );
        ...
        public virtual void SetExcelForm( ExcelPackage spreadSheet, DataGrid dataGrid )
        {
            if( spreadSheet != null
               && dataGrid?.ItemsSource != null )
            {
                try
                {
                    var _table = (DataTable)dataGrid.ItemsSource;
