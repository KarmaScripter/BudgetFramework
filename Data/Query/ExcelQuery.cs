...
namespace BudgetFramework
{
    ...
    public class ExcelQuery : Query
    {
        ...
        public void SaveFile( ExcelPackage workBook )
        {
            if( workBook != null )
            {
                try
                {
                    ...
                    if( !string.IsNullOrEmpty( _dialog?.SafeFileName ) )
                    {
        ...
        private void ExportToDataGrid( DataGrid dataGrid )
        {
            try
            {
                ...
                for( var i = 1; i <= _rows; i++ )
                {
                    for( var j = 1; j <= _columns; j++ )
                    {
                        ...
                        {
                            dataGrid.CurrentItem = _range[ i, j ].Address;
                        }
                    }
                }
